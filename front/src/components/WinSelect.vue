  ...
  <div class="win-select" ref="selectRef">
    ...
    <Teleport to="body">
      <Transition name="win-select-fade">
        <div v-if="isOpen" class="win-select-dropdown" :style="dropdownStyle" ref="dropdownRef">
          <div
          ...
          <div v-if="options.length === 0" class="win-select-empty">暂无数据</div>
        </div>
      </Transition>
    </Teleport>
  </div>
...
const handleClickOutside = e => {
  if (!selectRef.value?.contains(e.target) && !dropdownRef.value?.contains(e.target)) {
    isOpen.value = false
  }
}
